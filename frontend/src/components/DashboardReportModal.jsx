  ...
  const { data: reportData, isLoading, error, refetch } = useQuery(
    ...
    async () => {
      ...
      const [dashboardRes, statsRes, salesRes, purchasesRes, expensesRes, dayBookRes] = await Promise.all([
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/dashboard/day-book?startDate=${startDate}&endDate=${endDate}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}` }
        }).then(res => res.json()).catch(() => ({ data: [], summary: {} }))
      ]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg w-full max-w-4xl h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">{t('dashboardReport')}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 border-t bg-gray-50">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">{t('downloadReports')}</h3>
            <button
              onClick={onClose}
            ...
            {reportData && (
              <>
                ...
                <button
                  ...
                  className="px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 flex items-center justify-center gap-2 text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
